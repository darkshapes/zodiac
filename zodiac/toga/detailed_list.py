...
class ExampleDetailedListApp(toga.App):
    ...
    def on_select_handler(self, widget, **kwargs):
        ...
        self.label.text = f"Bee is {getattr(row, 'title', '')} in {getattr(row, 'subtitle', '')}" if row is not None else "No row selected"
...
def main():
    return ExampleDetailedListApp("Detailed List", "org.beeware.toga.examples.detailedlist")
